...
fun fastblur(sentBitmap: Bitmap, radius: Int): Bitmap? {
    val bitmap: Bitmap = sentBitmap.copy(sentBitmap.getConfig(), true)
    if (radius < 1) {
        return null
    }
...
fun MaterialDialog.blurEffects(activity : Activity) {
    val bitmapFast = takeScreenShot(activity)?.let { fastblur(it , 5) }
    val drawable = BitmapDrawable(activity.resources , bitmapFast)
